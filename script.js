...
function openArticle(index) {
  const story = index === 0 ? { category: 'Cultura', title: 'La generación que volvió a encontrarse sin una pantalla de por medio', image: 'https://images.unsplash.com/photo-1529156069898-49953e39b3ac?auto=format&fit=crop&w=1200&q=85', time: '6 min de lectura', body: 'Volver a conversar sin interrupciones se ha convertido en un pequeño acto de rebeldía. Visitamos una mesa larga, dejamos los teléfonos a un lado y descubrimos que la atención compartida todavía tiene mucho que decir.' } : stories[index];
  modalContent.innerHTML = `<div class="modal-hero"><img src="${story.image}" alt="${story.title}"><h2>${story.title}</h2></div><div class="modal-body"><span class="tag">${story.category} · ${story.time}</span><p>${story.body}</p><p>Porque las buenas historias no siempre piden prisa: a veces solo necesitan que les hagamos un poco de espacio.</p></div>`;
  modal.showModal();
}
